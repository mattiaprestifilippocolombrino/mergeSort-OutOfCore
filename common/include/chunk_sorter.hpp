...
struct ChunkData {
    ...
    std::string              runPath;  // path del file di run da creare
...
inline void freeChunk(ChunkData* chunk) {
    if (chunk == nullptr) {
        return;
    }
...
*/
inline void validateChunkBytes(size_t chunkBytes) {
    const size_t min_record = static_cast<size_t>(HEADER_SIZE) + PAYLOAD_MAX;
    if (chunkBytes < min_record) {
...
*/
inline void sortChunkAndWriteRun(ChunkData* chunk) {
    try {           // Uso la try-catch per catturare eventuali eccezioni.
        ...
        FILE* fout = std::fopen(chunk->runPath.c_str(), "wb");
        ...
        if (fout == nullptr) {
            std::fprintf(stderr, "[ERRORE] Impossibile creare %s\n",
                         chunk->runPath.c_str());
            chunk->errorFlag->store(true, std::memory_order_relaxed);
            freeChunk(chunk);
            return;
        }
...
// HEADER_SIZE + len. Si usa fwrite_unlocked per rimuovere l'overhead dei lock interni alla libc, essendo la scrittura svolta da un solo thread.
    ...
    } catch (...) {   // In caso di eccezione, viene settato il flag atomico per segnalare l'errore al thread principale.
        chunk->errorFlag->store(true, std::memory_order_relaxed);
    }
...
*/
inline std::vector<std::string> sortRangeToRuns(
    const std::string& inputPath, // Path del file di input.
    const std::string& tmpDir,    // Path della directory temporanea.
    size_t             chunkBytes,    // Dimensione dei chunk in byte.
    int64_t            startOffset, // Offset di inizio lettura. Viene usato nella versiono MPI per indicare il punto di partenza della porzione di file assegnata a ciascun rank.
    int64_t            endOffset,   // Offset di fine lettura. Viene usato nella versione MPI per indicare il punto di arrivo della porzione di file assegnata a ciascun rank.
    int                maxInflight = 0) // Controlla quanti task OpenMP possono essere “in volo”, cioè allocati e non ancora completati.
{
    ...
    if (fin == nullptr) {
        throw std::runtime_error("sort_to_runs: impossibile aprire " + inputPath);
    }
    ...
    if (fseeko(fin, static_cast<off_t>(startOffset), SEEK_SET) != 0) {
        std::fclose(fin);
    ...
    */
    int taskWindow;
    if (maxInflight > 0) {
        taskWindow = maxInflight;
    } else {
        taskWindow = std::max(2, 2 * omp_get_max_threads());
    }
...
*/    
    ...
    #pragma omp single
    {
        ...
        int inFlight = 0;  // Numero di task in volo attuali.
        ...
        while (true) {
            ...
            bool eofReached     = false;       // Indica se e' stata raggiunta la fine del file.

            try {
                // Per ogni iterazione viene creato un nuovo chunk. 
                chunk              = new ChunkData(); // Alloco un buffer grande quanto il chunk richiesto. 
                chunk->buffer      = new char[chunkBytes]; // Il buffer contiene record completi, mai record spezzati.
                chunk->errorFlag  = &errorFlag; // Flag condiviso di errore-
                size_t bufUsed    = 0; 
                ...
                chunk->index.reserve(estimatedRecords);    // Alloca memoria sufficiente per contenere il numero stimato di record.
                ...
                while (true) {
                    ...
                    RecordHeader hdr;
                    bool gotRecord = readHeader(fin, hdr);  // Legge l'header con l'apposita funzione. 
                    if (!gotRecord) {
                        eofReached = true;
                        break; // EOF
                    }
...
*/
                ...
                ++inFlight;   //Si incrementa il contatore dei task lanciati.
                ...
                */
                if (inFlight >= taskWindow) {
                    #pragma omp taskwait   // Attende che tutti i task in esecuzione siano completati.
                    inFlight = 0;  // Resetta il contatore dei task in esecuzione.
                }
...
inline std::vector<std::string> sortToRuns(
    const std::string& inputPath,
    const std::string& tmpDir,
    size_t             chunkBytes)
{
    return sortRangeToRuns(inputPath, tmpDir, chunkBytes, 0, -1);
}
