...
Per la relazione non lanciare tutto insieme: dividi per dominio e tieni ogni job intorno a 10-20 minuti. I casi consigliati sono:
...
`manySmall50M:50000000:64` resta utile come misura extra, ma dai risultati osservati una sola run OpenMP a 1 thread dura gia' circa 34 s; con due implementazioni, molti thread e piu' trial supera facilmente il limite pratico del cluster.
...
Lo script Slurm single-node usa default brevi e adatti al cluster:
...
Esempio OpenMP single-node principale per speedup ed efficienza:
...
Esempio FastFlow separato, da aggiungere allo stesso CSV:
...
Esempio extra con 50M record, da lanciare in job separate e con un solo trial:
...
- Nel job MPI il default esegue solo strong scaling. Imposta `RUN_STRONG=0 RUN_WEAK=1` per eseguire weak scaling in una job separata.
