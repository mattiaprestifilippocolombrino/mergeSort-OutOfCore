...
for spec in $BENCHMARK_CASES; do
    ...
    for trial in $(seq 1 "$TRIALS"); do
        for threads in $THREAD_LIST; do
            ...
            run_impl "omp" "$name" "$input" "$records" "$payload" "$threads" "$trial"

            if [[ -x "$BUILD_DIR/ff_sort" ]]; then
                log "FastFlow case=$name trial=$trial workers=$threads"
                run_impl "ff" "$name" "$input" "$records" "$payload" "$threads" "$trial"
            elif [[ "$trial" == "1" && "$threads" == "$(awk '{print $1; exit}' <<<"$THREAD_LIST")" ]]; then
